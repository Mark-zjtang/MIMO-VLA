...
class BirdeyeRender(object):
  def __init__(self, world, params):
    ...
    #Add render camera_route
    self.camera_route_waypoints_surface = pygame.Surface((self.map_image.surface.get_width(), self.map_image.surface.get_height()))
    self.camera_route_waypoints_surface.set_colorkey(COLOR_BLACK)
    
    scaled_original_size = self.original_surface_size * (1.0 / 0.62)
    self.hero_surface = pygame.Surface((scaled_original_size, scaled_original_size)).convert()
    self.hero_route_surface = pygame.Surface((scaled_original_size, scaled_original_size)).convert()
  ...
  def render_waypoints(self, surface, waypoints, world_to_pixel):
    ...
    for p in waypoints:
      ...
      corners.append(carla.Location(x=p[0],y=p[1], z=0))
  ...
  def render_camera_route(self,surface, waypoints, world_to_pixel):
    if self.red_light:
      # purple
      color = pygame.Color(math.floor(0.5 * 255), 0, math.floor(0.5 * 255))
    else:
    ...
    pygame.draw.lines(surface, color, False, corners, 120)
  ...
  def render(self, display, render_types=None):
    ...
    self.camera_route_waypoints_surface.fill(COLOR_BLACK)
    self.render_camera_route(
      self.camera_route_waypoints_surface,
      self.waypoints, self.map_image.world_to_pixel)
    ...
    if self.hero_actor is not None:
      ...
      #Add hero_camera_route surface
      self.hero_route_surface.fill(COLOR_BLACK)
      self.camera_route_surface = self.camera_route_waypoints_surface
      self.camera_route_surface.set_clip(clipping_rect)
      self.hero_route_surface.blit(self.camera_route_surface, (-translation_offset[0],
                             -translation_offset[1]))
      ...
      rotated_camera_route_surface = pygame.transform.rotozoom(self.hero_route_surface, angle, 1.0).convert()

      center = (display.get_height() / 2 , display.get_height() / 2)
      camera_toute_center = (display.get_height() / 2 + display.get_height() * 2, display.get_height() / 1.2)
      rotation_pivot = rotated_result_surface.get_rect(center=center)
      rotation_pivot_camera_route = rotated_result_surface.get_rect(center=camera_toute_center)
      display.blit(rotated_result_surface, rotation_pivot)
      display.blit(rotated_camera_route_surface, rotation_pivot_camera_route)
      ...
      display.blit(self.result_surface, (translation_offset[0] + center_offset[0],
                         translation_offset[1] + center_offset[0]))