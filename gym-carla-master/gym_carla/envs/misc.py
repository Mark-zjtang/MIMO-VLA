...
def get_info(vehicle):
  ...
  vx = vel.x
  vy = vel.y
  info = (x, y, yaw, l, w, vx, vy)
  return info


def get_local_pose_and_velo(global_pose_and_velo, ego_pose_and_velo):
  """
  Transform vehicle to ego coordinate
  :param global_pose_and_velo: surrounding vehicle's global pose and velocity
  :param ego_pose_and_velo: ego vehicle pose and velocity
  :return: tuple of the pose and velocity of the surrounding vehicle in ego coordinate
  """
  x, y, yaw, vx, vy = global_pose_and_velo
  ego_x, ego_y, ego_yaw, ego_vx, ego_vy = ego_pose_and_velo
  R = np.array([[np.cos(ego_yaw), np.sin(ego_yaw)],
                [-np.sin(ego_yaw), np.cos(ego_yaw)]])
  vec_local = R.dot(np.array([x - ego_x, y - ego_y]))
  velo_local = R.dot(np.array([vx - ego_vx, vy - ego_vy]))
  yaw_local = yaw - ego_yaw
  local_pose_and_velo = (vec_local[0], vec_local[1], yaw_local, velo_local[0], velo_local[1])
  return local_pose_and_velo
...
def is_within_distance_around(target_vector, target_vehicle, orientation, norm_target):
  # print("orientation", orientation)

  v = target_vehicle.get_velocity()
  target_speed = np.sqrt(v.x ** 2 + v.y ** 2)
  forward_vector = np.array([math.cos(math.radians(orientation)), math.sin(math.radians(orientation))])
  d_angle = math.degrees(math.acos(np.dot(forward_vector, target_vector) / norm_target))

  return np.round((norm_target, d_angle, target_speed), decimals=2)
